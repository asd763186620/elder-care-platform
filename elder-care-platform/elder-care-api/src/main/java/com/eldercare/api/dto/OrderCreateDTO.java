...
public record OrderCreateDTO(
        ...
        // 服务地址；第一版直接保存文本地址。
        @NotBlank(message = "不能为空") String serviceAddress,
        // 预约服务开始时间；必须早于 serviceEndTime。
        @NotNull(message = "不能为空") LocalDateTime serviceStartTime,
        // 预约服务结束时间；必须晚于 serviceStartTime。
        @NotNull(message = "不能为空") LocalDateTime serviceEndTime,
        // 派单模式；ASSIGNED 表示指定志愿者，PUBLIC 表示进入公共订单池。
        @NotBlank(message = "不能为空") String assignMode,
