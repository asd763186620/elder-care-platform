...
/**
 * 用户服务业务实现。
 * 说明：登录、Token、退出登录、手机号绑定和身份切换已迁移到 elder-care-auth-service。
 */
@Service
public class UserAppServiceImpl implements UserAppService {
    ...
    @OperationLog(module = OperationModuleEnum.ELDER, operationType = OperationTypeEnum.CREATE,
            description = "新增老人资料", bizId = "#createDTO.userId")
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    @OperationLog(module = OperationModuleEnum.FAMILY_BINDING, operationType = OperationTypeEnum.BIND,
            description = "亲情号绑定老人", bizId = "#bindDTO.elderUserId")
    ...
    private UserAccount resolveElderAccount(Long communityId, ElderCreateDTO createDTO) {
        ...
        // 根据手机号查找或创建账号。
        return resolveAccountByPhone(communityId, createDTO.phone());
    }

    /**
     * 根据手机号查找或创建社区内账号。
     */
    private UserAccount resolveAccountByPhone(Long communityId, String phone) {
        // 根据手机号查询账号。
        ...
        // 设置昵称。
        newAccount.setNickname("用户" + phone);
        // 设置未知性别。
    ...
    private ElderProfileVO toElderProfileVO(ElderProfile elderProfile) {
        // 组装接口返回对象。
        return new ElderProfileVO(elderProfile.getId(), elderProfile.getUserId(), elderProfile.getElderName(),
                elderProfile.getElderPhone(), elderProfile.getAge(), elderProfile.getAddress(), elderProfile.getHealthNote());
    }
}
